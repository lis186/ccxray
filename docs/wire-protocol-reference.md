...
| Codex tool-result exit code | In Codex JSON-in-string tool output, only a top-level safe-integer `exit_code` is decoded. `0` is clean; any non-zero integer, including a negative integer, is failure; fractional or unsafe numeric values are undecodable | `obs-fragile` Codex 0.145.0 (CLI payload) / `obs-stable` ccxray ≥2.3.0 (ccxray convention) |
