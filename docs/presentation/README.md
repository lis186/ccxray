...
| 三 · 第一張 X 光片 | 3:20–5:10 | **dashboard 導覽七頁(總—分鏡頭)**:完整畫面 → 圈 topbar → 放大(STEP 2)→ 拉回圈 columns → 放大(STEP 3)→ 拉回圈 timeline → 放大(STEP 4)——同一張全圖 zoom,空間關係不斷裂 → **LIVE DEMO**:點新 session 的 turn #1 → System 分塊 → **context 稅導覽六頁(STEP 5)**:回放同一畫面 → 圈第 1 個 turn → 圈 Cost → 放大(In 22K/$0.07)→ 圈 System → 放大([user] 佔大頭) | 導覽每頁 3–5 秒,「圈」頁指著說、「放大」頁講內容;demo 是全場核心 aha:「你只說了一句話,22K 先付了」;稅導覽收在三連擊「全是行前簡報 → 大部分是你自己塞進去的 → 用不到的就刪掉」 |
...
| 六 · 歸返 | 8:40–9:20 | 回扣二月底那個問題:上限=行前簡報+斷 cache+塞爆 context+選錯模型;X 光哲學 | 「治不了病」重擊 |
...
- **Fallback**:demo 任何一步卡住,直接翻下一頁——STEP 5 六頁導覽就是同畫面截圖的操作路徑回放
  (圈第 1 個 turn → 圈 Cost → 放大 → 圈 System → 放大),講稿照走,零損傷。
...
STEP 1 架構分鏡×5(FLIP)| 2–4 dashboard 導覽分鏡×7(全圖→圈選→放大,同一張全圖 zoom)| **5 turn #1 context 稅導覽分鏡×6(demo fallback;圈 turn→圈 Cost→放大→圈 System→放大)**
