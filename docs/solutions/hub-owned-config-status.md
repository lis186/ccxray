...
| 8 | old client × new hub, new client × old hub | no crash; new client is silent against an old hub. **Same-major skew on first attach only.** `checkVersionCompat` (`hub.js:158-171`) runs at first attach (`index.js:1039-1043`) and exits fatally on a major mismatch — but reviewer A found the recovery re-register (`index.js:1144`) runs NO version check, and `forkHub` re-spawns from on-disk code that may have been upgraded under a live client. So cross-major replies ARE reachable, on row 9's path precisely. Test them there. Cover BOTH the socket and the HTTP register fallback (`discoverHub`), which reviewer A found returns `null` via the 410 tombstone rather than a reply. **Disposition (2026-08-29)**: the new-client × old-hub half is built at the status-CLI surface as a spawned e2e over both carriers, and at the register-reply surface in-process (`test/hub-client-status.test.js` full/old-hub/null-tombstone shape table + loud-env negative); a register-carrier e2e against a genuinely old hub binary, and the old-client half, are deferred — arguments in the deferred-rows note below |
