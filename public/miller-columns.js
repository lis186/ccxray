...
function countSkillInvocations(messages, loadedSkills) {
	...
	for (var mi = 0; mi < (messages || []).length; mi++) {
		...
		if (!Array.isArray(blks))
			blks = [{ type: "text", text: String(blks || "") }];
		for (var bi = 0; bi < blks.length; bi++) {
			if (
				blks[bi].type === "tool_use" &&
				blks[bi].name === "Skill" &&
				blks[bi].input?.skill
			)
	...
	for (var mi2 = 0; mi2 < (messages || []).length; mi2++) {
		...
		if (!Array.isArray(blks2))
			blks2 = [{ type: "text", text: String(blks2 || "") }];
...
const SP_BLOCK_OWNERS = {
	billingHeader: "anthropic",
	coreIdentity: "anthropic",
	coreInstructions: "anthropic",
	customSkills: "user",
	pluginSkills: "user",
	mcpServersList: "user",
	settingsJson: "user",
...
function renderSystemBlockViewer(system) {
	...
	if (typeof system === "string") {
		return (
			'<pre style="margin:0;font-size:11px;font-family:monospace;line-height:1.5;white-space:pre-wrap;word-break:break-word">' +
			escapeHtml(system) +
			"</pre>"
		);
	}
	if (!Array.isArray(system) || system.length < 3) {
		return (
			'<pre style="margin:0;font-size:11px;font-family:monospace;line-height:1.5;white-space:pre-wrap;word-break:break-word">' +
			escapeHtml(JSON.stringify(system, null, 2)) +
			"</pre>"
		);
	}
	...
	var html =
		'<div style="font-size:11px;color:var(--dim);margin-bottom:8px">System Prompt (this turn) &nbsp; <span style="color:var(--text)">' +
		(totalLen / 1000).toFixed(1) +
		"k</span> chars</div>";
	...
	for (var i = 0; i < blockOrder.length; i++) {
		...
		html +=
			"<details" +
			(isOpen ? " open" : "") +
			' style="margin-bottom:4px;border:1px solid var(--border);border-radius:4px">';
		html +=
			'<summary style="padding:6px 10px;cursor:pointer;font-size:11px;display:flex;align-items:center;gap:8px;background:var(--surface)">';
		html +=
		...
		html +=
			'<span style="font-size:9px;padding:1px 5px;border:1px solid ' +
			ownerColor +
			";color:" +
			ownerColor +
			';border-radius:3px">' +
		...
		html += "</summary>";
		html +=
			'<pre style="margin:0;padding:8px 10px;font-size:10px;font-family:monospace;line-height:1.4;white-space:pre-wrap;word-break:break-word;max-height:400px;overflow-y:auto">' +
			escapeHtml(text) +
			"</pre>";
		html += "</details>";
	}
...
const pinnedProjects = new Set(
	JSON.parse(localStorage.getItem("xray-pinned-projects") || "[]"),
);
const pinnedSessions = new Map(); // sid → { sid, pinnedAt }
JSON.parse(localStorage.getItem("xray-pinned-sessions") || "[]").forEach((p) =>
	pinnedSessions.set(p.sid, p),
);

function savePinnedProjects() {
	localStorage.setItem(
		"xray-pinned-projects",
		JSON.stringify([...pinnedProjects]),
	);
}
function savePinnedSessions() {
	localStorage.setItem(
		"xray-pinned-sessions",
		JSON.stringify([...pinnedSessions.values()]),
	);
}
...
function expireSessionPins() {
	...
	for (const [sid, pin] of pinnedSessions) {
		...
		if (sess && sess.lastId && sess.lastId.length >= 19) {
			const ts = new Date(
				sess.lastId.slice(0, 10) +
					"T" +
					sess.lastId.slice(11, 19).replace(/-/g, ":"),
			).getTime();
			if (ts) lastActive = ts;
		}
...
let projectFilterMode =
	sessionStorage.getItem("xray-project-filter") || "active";
...
let sessionFilterMode =
	sessionStorage.getItem("xray-session-filter") || "active+idle";
...
function applySessionFilter() {
	colSessions.querySelectorAll(".session-item").forEach((el) => {
		...
		if (selectedProjectName) {
			...
			if (projName !== selectedProjectName) {
...
function getStatusClass(sid) {
	...
	if (s.lastSeenAt && Date.now() - s.lastSeenAt < 5 * 60 * 1000)
		return "sdot-idle";
	return "sdot-off";
}
...
function updateTopbarStatus() {
	const streaming = [...sessionStatusMap.values()].filter(
		(s) => s.active,
	).length;
	const idle = [...sessionStatusMap.values()].filter(
		(s) =>
	...
	if (streaming)
		txt +=
			'<span style="color:var(--green)">●' + streaming + " streaming</span>  ";
	if (idle)
		txt += '<span style="color:var(--yellow)">◐' + idle + " idle</span>";
...
function toggleFollowLive() {
	...
	if (btn) {
		...
		btn
			.querySelector(".scroll-off")
			.classList.toggle("active", !followLiveTurn);
	}
}
...
function formatRelativeTime(id) {
	...
	const ts = new Date(
		id.slice(0, 10) + "T" + id.slice(11, 19).replace(/-/g, ":"),
	).getTime();
...
function copyLaunchCmd(sid, btn) {
	...
	const cmd =
		"ANTHROPIC_BASE_URL=http://localhost:" + port + " claude --continue " + sid;
...
function renderSessionItem(sess, sid) {
	...
	const shortModel = (sess.model || "?")
		.replace("claude-", "")
		.replace(/-[0-9]{8}$/, "");
	const costStr = sess.totalCost > 0 ? "$" + sess.totalCost.toFixed(3) : "—";
	const dateStr = sess.lastId
		? formatRelativeTime(sess.lastId)
		: sess.firstId
			? formatEntryDate(sess.firstId)
			: escapeHtml(sess.firstTs || "");
	const totalCalls = Object.values(sess.toolCalls || {}).reduce(
		(s, n) => s + n,
		0,
	);
	...
	const toolRow =
		totalCalls > 0
			? '<div class="si-tools">' +
				(topTools || totalCalls + " calls") +
				"</div>"
			: "";
	const ctxPct = sess.latestMainCtxPct || 0;
	const ctxAlertHtml =
		ctxPct >= 90
			? '<span class="ctx-alert ctx-alert-red">' +
				Math.round(ctxPct) +
				"%</span>"
			: ctxPct >= 80
				? '<span class="ctx-alert ctx-alert-yellow">' +
					Math.round(ctxPct) +
					"%</span>"
				: "";
	...
	const sdotClasses =
		"sdot " + getStatusClass(sid) + (isArmed ? " sdot-armed" : "");
	const sdotTitle = !isOnline
		? ""
		: isArmed
			? "Intercept armed · click to disable"
			: "Click to arm intercept";
	const sdotOnclick = isOnline
		? "event.stopPropagation();toggleIntercept(&quot;" +
			escapeHtml(sid) +
			"&quot;)"
		: "";
	const heldHtml = isHeld
		? '<span class="held-badge" onclick="event.stopPropagation();showInterceptOverlay()">HELD</span>'
		: "";
	...
	return (
		...
		sdotClasses +
		'"' +
		(sdotTitle ? ' title="' + sdotTitle + '"' : "") +
		(sdotOnclick ? ' onclick="' + sdotOnclick + '"' : "") +
		' tabindex="-1"></button>' +
		'<span class="sid">' +
		escapeHtml(shortSid) +
		"</span>" +
		pinBtn +
		'<button class="launch-btn" onclick="event.stopPropagation();copyLaunchCmd(&quot;' +
		escapeHtml(sid) +
		'&quot;,this)" title="Copy launch cmd">&#8855;</button>' +
		heldHtml +
		"</div>" +
		'<div class="si-row2">' +
		...
		" · " +
		sess.count +
		't · <span class="si-cost">' +
		escapeHtml(costStr) +
		"</span></div>" +
		toolRow +
		'<div class="si-row3"><span title="' +
		escapeHtml(sess.lastId ? formatEntryDate(sess.lastId) : "") +
		'">' +
		dateStr +
		"</span>" +
		ctxAlertHtml +
...
function renderProjectsCol() {
	let html =
		'<div class="col-title" style="display:flex;align-items:center;gap:6px">Projects' +
		...
		'<option value="active"' +
		(projectFilterMode === "active" ? " selected" : "") +
		">Active</option>" +
		'<option value="all"' +
		(projectFilterMode === "all" ? " selected" : "") +
		">All</option>" +
		"</select></div>";
	...
	for (const proj of sorted) {
		...
		if (projectFilterMode === "active") {
			...
			if (
				!isPinned &&
				!isSel &&
				(isSystemProject(proj.name) || statusClass === "sdot-off")
			)
				continue;
		}
		const isSel = selectedProjectName === proj.name;
		...
		const pinBtn =
			'<button class="pin-btn' +
			(isPinned ? " pinned" : "") +
			'" onclick="event.stopPropagation();togglePinProject(' +
			JSON.stringify(proj.name).replace(/"/g, "&quot;") +
			')" title="' +
			(isPinned ? "Unpin" : "Pin") +
			'">★</button>';
		html +=
			...
			JSON.stringify(proj.name).replace(/"/g, "&quot;") +
			')">' +
			'<div class="pi-name"><span class="sdot ' +
			statusClass +
			'"></span>' +
			escapeHtml(truncateMiddle(proj.name, 20)) +
			pinBtn +
			"</div>" +
			'<div class="pi-meta">' +
			proj.sessionIds.size +
			" sessions</div>" +
			'<div class="pi-meta pi-cost">$' +
			proj.totalCost.toFixed(3) +
			"</div>" +
			(rangeStr
				? '<div class="pi-range">' + escapeHtml(rangeStr) + "</div>"
				: "") +
			"</div>";
	}
...
function selectProject(name) {
	...
	selectedProjectName =
		name !== null && name === selectedProjectName ? null : name;
	renderProjectsCol();
	applySessionFilter();
...
function getVisibleTurnIndices() {
	return allEntries
		.map((e, i) => i)
		.filter(
			(i) => selectedSessionId && allEntries[i].sessionId === selectedSessionId,
		);
}
	...
	const chips = sorted
		...
		.map(
			([n, c]) =>
				...
				escapeHtml(n.replace(/^mcp__[^_]+__/, "")) +
				"·" +
				c +
				"</span>",
		)
...
function renderSessionSparkline(sid) {
	...
	const turns = allEntries.filter(
		(e) =>
			e.sessionId === sid &&
			!e.isSubagent &&
			e.usage &&
			(e.usage.input_tokens || 0) > 0,
	);
	if (turns.length < 1) {
		el.style.display = "none";
		return;
...
function renderBarChart(el, turns) {
	const W = 400,
		...
		PAD = 4;
	const maxCtx =
		turns.reduce((m, e) => Math.max(m, e.maxContext || DEFAULT_MAX_CTX), 0) ||
		DEFAULT_MAX_CTX;
	...
	svg +=
		...
		'" y2="' +
		threshY +
		'" stroke="var(--dim)" stroke-width="0.5" stroke-dasharray="4 2"/>';

	turns.forEach((e, i) => {
		const ctx = e.ctxUsed || 0;
		const pct = Math.min(100, (ctx / maxCtx) * 100);
		const color =
			pct > 90 ? "var(--red)" : pct > 80 ? "var(--yellow)" : "var(--accent)";
		const x = PAD + i * barW;
		...
		if (e.isCompacted) {
			svg +=
				...
				'" width="' +
				(barW - gap).toFixed(1) +
				'" height="3" fill="var(--red)" opacity="0.9"/>';
		}
	});
	svg += "</svg>";
	el.innerHTML = svg;
...
function predictRemainingTurns(sid) {
	...
	for (let i = 1; i < window.length; i++) {
		deltas.push(
			(window[i].tokens.messages || 0) - (window[i - 1].tokens.messages || 0),
		);
	}
	...
	const currentTotal =
		(last.tokens.system || 0) +
		(last.tokens.tools || 0) +
		(last.tokens.messages || 0);
...
function computeSessionScorecard(sid) {
	...
	const toolUtilization =
		availableTools > 0 ? (usedTools.size / availableTools) * 100 : 0;
...
function showScorecard(sessItem) {
	...
	function bar(pct, color) {
		return (
			'<div class="sc-bar"><div class="sc-bar-fill" style="width:' +
	...
	function row(label, value, pct, color) {
		return (
			...
			label +
			'</span><span class="sc-value" style="color:' +
			color +
			'">' +
			value +
			"</span></div>" +
	...
	const chColor =
		sc.cacheHitRate >= 80
			...
			: sc.cacheHitRate >= 50
				...
				: "var(--red)";
	const ceColor =
		sc.contextEfficiency >= 50 ? "var(--accent)" : "var(--yellow)";
	const tuColor = sc.toolUtilization >= 30 ? "var(--green)" : "var(--yellow)";
	const ccColor = sc.compressionCount === 0 ? "var(--green)" : "var(--yellow)";

	el.innerHTML =
		'<div style="font-weight:bold;margin-bottom:6px;font-size:11px">Session Scorecard</div>' +
		row(
			"Context Efficiency",
			sc.contextEfficiency.toFixed(0) + "%",
			sc.contextEfficiency,
			ceColor,
		) +
		row(
			"Cache Hit Rate",
			sc.cacheHitRate.toFixed(0) + "%",
			sc.cacheHitRate,
			chColor,
		) +
		'<div class="sc-row"><span class="sc-label">Compressions</span><span class="sc-value" style="color:' +
		ccColor +
		'">' +
		sc.compressionCount +
		"</span></div>" +
...
function renderStackedAreaChart(el, turns) {
	...
	for (let layerIdx = layers.length - 1; layerIdx >= 0; layerIdx--) {
		...
		const d =
			"M" + topPoints.join(" L") + " L" + botPoints.reverse().join(" L") + " Z";
		svg +=
			'<path d="' +
			d +
			'" fill="' +
			layers[layerIdx].color +
			'" opacity="0.8"/>';
	}
	...
	turns.forEach((e, i) => {
		if (e.isCompacted) {
			...
			svg +=
				...
				'" y2="' +
				(H - PAD) +
				'" stroke="var(--red)" stroke-width="1.5" stroke-dasharray="3 2" opacity="0.9"/>';
		}
	});
		...
		if (lastPct < 0.95) {
			...
			if (count > 0) {
				...
				svg +=
					...
					xPos(n - 1).toFixed(1) +
					'" y1="' +
					yPos(lastTotal).toFixed(1) +
					'" x2="' +
					clampX +
					'" y2="' +
					yPos(maxCtx).toFixed(1) +
					'" stroke="var(--dim)" stroke-width="1" stroke-dasharray="4 2" opacity="0.6"/>';
			}
		}
	}
...
function renderBreadcrumb() {
	...
	const projName =
		selectedProjectName ||
		(selectedSessionId &&
			sessionsMap.get(selectedSessionId) &&
			getProjectName(sessionsMap.get(selectedSessionId).cwd));
	if (projName)
		segments.push({
			label: projName,
			action: () => {
	...
	if (selectedSessionId)
		segments.push({
			label: "session:" + selectedSessionId.slice(0, 8),
			action: () => {
				selectSessionAndLatestTurn(selectedSessionId);
			},
		});
	if (selectedTurnIdx >= 0) {
		...
		const sessEl = e
			? colTurns.querySelector(
					'.turn-item[data-entry-idx="' + selectedTurnIdx + '"]',
				)
			: null;
...
function selectTurn(idx) {
	...
	const selEl = colTurns.querySelector(
		'.turn-item[data-entry-idx="' + idx + '"]',
	);
...
function renderSectionsCol(idx) {
	const e = allEntries[idx];
	if (!e) {
		colSections.innerHTML = '<div class="col-empty">No data</div>';
		return;
	}
	const tok = e.tokens || {};
	const req = e.req || {};
	...
	const statusClass =
		e.status >= 200 && e.status < 300 ? "status-ok" : "status-err";
	const resEvents = Array.isArray(e.res) ? e.res : [];
	const stopReason =
		e.stopReason ||
		(Array.isArray(resEvents)
			? resEvents.find((ev) => ev.type === "message_delta")?.delta
					?.stop_reason || ""
			: "");
	const turnCost = e.cost;
	const shortModel = (e.model || "?")
		.replace("claude-", "")
		.replace(/-[0-9]{8}$/, "");
	...
	html +=
		'<div class="ch-line1"><span style="color:var(--dim)">' +
		(isSubagent ? "" : "#") +
		escapeHtml(displayNum) +
		'</span> <span style="color:var(--purple)">' +
		escapeHtml(shortModel) +
		"</span>" +
		subBadge +
		inferBadge +
	...
	html +=
		'<div class="ch-line2" style="margin-top:2px">' +
		fmt(inTok) +
		" in / " +
		fmt(outTok) +
		" out";
	...
	if (tok?.contextBreakdown) {
		...
		html +=
			'<div style="padding:4px 12px 6px;border-bottom:1px solid var(--border)"><div style="height:8px;border-radius:2px;background:var(--border);margin:4px 0 2px"></div><div style="height:12px;width:80px;border-radius:2px;background:var(--border)"></div></div>';
	}

	const coreTools = req.tools
		? req.tools.filter((t) => !t.name.startsWith("mcp__"))
		: null;
	const mcpTools = req.tools
		? req.tools.filter((t) => t.name.startsWith("mcp__"))
		: null;
	const tc = allEntries[idx]?.toolCalls || {};
	const coreCalls = Object.entries(tc)
		.filter(([n]) => !n.startsWith("mcp__"))
		.reduce((s, [, c]) => s + c, 0);
	const mcpCalls = Object.entries(tc)
		.filter(([n]) => n.startsWith("mcp__"))
		.reduce((s, [, c]) => s + c, 0);

	// Extract skill usage from messages (all 3 paths: user /cmd, model Skill tool, hybrid)
	const skillCalls = e.reqLoaded
		? countSkillInvocations(req.messages, tok.contextBreakdown?.loadedSkills)
		: {};
	...
	const ccVer =
		req.system && Array.isArray(req.system) && req.system[0]
			? (req.system[0].text || "").match(/cc_version=(S+?)[; ]/)?.[1]
			: null;
	...
	const previewSteps = e.reqLoaded
		? getCachedSteps(req.messages, resEvents)
		: [];
	const stepCount = previewSteps.length;
	const stepErrorCount = previewSteps.filter(
		(s) => s.type === "tool-group" && s.calls.some((c) => c.isError),
	).length;
	...
	function renderSectionItem(s) {
		...
		const dot = s.color
			...
			: '<span style="display:inline-block;width:7px;margin-right:5px"></span>';
		h += '<span class="si-name">' + dot + s.label + "</span>";
		if (s.badge)
			h += '<span class="si-badge">' + escapeHtml(s.badge) + "</span>";
	...
	const contextSections = [
		...
		},
		{
			name: "mcp-tools",
			label: "MCP",
			color: "var(--color-mcp-tools)",
			badge: mcpTools
				? mcpTools.length + " tools" + (mcpCalls ? " · " + mcpCalls + "×" : "")
				: e.reqLoaded
					? ""
					: "…",
			extra: "",
	...
	const hasSkillTool = e.reqLoaded
		? !!req.tools?.some((t) => t.name === "Skill")
		: false;
	...
	html += renderSectionItem({
		...
		label: "Events",
		color: null,
		badge: resEvents.length ? resEvents.length + " events" : "",
		extra: "",
	});
	if (!e.reqLoaded)
		html +=
...
function renderCostEfficiencyPanel(currentEntry) {
	...
	const sessionTurns = allEntries.filter(
		(e) => e.sessionId === sid && !e.isSubagent && e.usage,
	);
	...
	html +=
		'<div style="font-size:13px;font-weight:bold;margin-bottom:12px">Cost Efficiency</div>';

	// Cache hit rate bar
	html += '<div style="margin-bottom:12px">';
	html +=
		'<div style="font-size:11px;color:var(--dim);margin-bottom:4px">Cache Hit Rate</div>';
	if (totalCache > 0) {
		html +=
			'<div style="display:flex;height:10px;border-radius:3px;overflow:hidden;background:var(--border);margin-bottom:4px">';
		html +=
			'<div style="width:' +
			hitRate.toFixed(1) +
			"%;background:" +
		...
		html +=
			'<div style="font-size:11px;color:var(--dim)">No cache data for this session</div>';
	}
	html += "</div>";
	...
	html +=
		'<div id="' +
		savingsId +
		'" style="font-size:11px;color:var(--dim);margin-bottom:16px">Calculating savings…</div>';
	...
	if (cb) {
		html +=
			'<div style="font-size:11px;color:var(--dim);margin-bottom:4px">Fixed Cost per Turn (System + Tools)</div>';
		...
		const systemSections = [
			...
			{
				label: "Settings/Env/Git",
				tokens: (sb.settingsJson || 0) + (sb.envAndGit || 0),
			},
			...
			{
				label: "Core identity",
				tokens: (sb.coreIdentity || 0) + (sb.billingHeader || 0),
			},
		]
			.filter((s) => s.tokens > 0)
		...
		const toolSections = [
			{
				label: "Core tools",
				tokens: coreToolTokens,
				count: cb.toolsBreakdown?.counts
					? Object.entries(cb.toolsBreakdown.counts)
							.filter(([k]) => k !== "mcp")
							.reduce((s, [, v]) => s + v, 0)
					: 0,
				color: "var(--color-tools)",
		...
		const maxLabelLen = Math.max(
			...allBars.filter(Boolean).map((s) => s.label.length),
			1,
		);
		...
		for (const s of allBars) {
			...
			html +=
				'<span style="width:' +
				labelW +
				'px;flex-shrink:0;color:var(--text)">' +
				escapeHtml(s.label) +
				"</span>";
			html +=
				'<div style="flex:1;height:6px;border-radius:2px;background:var(--border)"><div style="width:' +
				barW +
				"%;height:100%;border-radius:2px;background:" +
				s.color +
				'"></div></div>';
			html +=
				'<span style="width:50px;text-align:right;flex-shrink:0;color:var(--dim)">' +
		...
		html +=
			'<div style="background:var(--border);border-radius:4px;padding:8px;font-size:11px;margin-bottom:12px">';
		html +=
			"Fixed tax per turn: <strong>" +
			fmt(fixedTax) +
			" tokens</strong> = " +
			fixedPct +
			"% of " +
		...
		if (mcpPlugins.length) {
			...
			const unusedPlugins = mcpPlugins.filter((p) => {
				...
				return !Object.keys(sessionToolCalls).some((name) =>
					name.startsWith("mcp__" + p.plugin + "__"),
				);
			});
			for (const p of unusedPlugins) {
				html +=
					'<div style="padding:2px 0;font-size:11px;color:var(--yellow)">';
				html +=
					"⚠ " +
					escapeHtml(p.plugin) +
					" has " +
					p.count +
					" tools but 0 uses this session (" +
		...
		html +=
			'<div style="font-size:11px;color:var(--dim)">Load request data to see full analysis</div>';
	}
	...
	fetchPricingData()
		.then((pricing) => {
			...
			const normalCost =
				(totalCacheRead / 1_000_000) * rates.input_cost_per_mtok;
			const cacheCost =
				(totalCacheRead / 1_000_000) * rates.cache_read_cost_per_mtok;
			const saved = normalCost - cacheCost;
			el.innerHTML =
				'Cache savings this session: <strong style="color:var(--green)">$' +
				saved.toFixed(3) +
				"</strong>";
		})
		.catch(() => {
...
function renderDetailCol() {
	...
	if (!selectedSection) {
		commitDetailHtml(
			'<div class="detail-scroll"><div class="col-empty"><div class="col-empty-hint">← Select a section</div></div></div>',
		);
		return;
	}
	if (!e) {
		commitDetailHtml(
			'<div class="detail-scroll"><div class="col-empty">No data</div></div>',
		);
		return;
	}
	...
	if (selectedSection === "timeline" || isFocusedMode) {
		...
		headerHtml =
			'<div class="col-header" style="display:flex;align-items:center;justify-content:space-between;padding:8px 12px;border-bottom:1px solid var(--border)">' +
			'<span style="font-size:11px;font-weight:600;color:var(--dim);text-transform:uppercase;letter-spacing:0.08em">' +
			escapeHtml(sectionLabel) +
			"</span>" +
	...
	switch (selectedSection) {
		case "system":
			if (req.system) {
				inner =
					...
					renderSystemBlockViewer(req.system) +
					"</div>";
			} else {
				inner = e.reqLoaded
					? '<div class="col-empty">No system prompt</div>'
					: loading;
			}
			break;
		case "timeline": {
			if (!isFocusedMode) {
				...
				if (!currentSteps.length) {
					inner = e.reqLoaded
						? '<div class="col-empty">No messages</div>'
						: loading;
					break;
				}
				const toolFreqPreview = {};
				currentSteps.forEach((s) => {
					if (s.type === "tool-group")
						s.calls.forEach((c) => {
							toolFreqPreview[c.name] = (toolFreqPreview[c.name] || 0) + 1;
						});
				});
				const totalPreview = currentSteps.filter(
					(s) => s.type === "tool-group",
				).length;
				const errorPreview = currentSteps.filter(
					(s) => s.type === "tool-group" && s.calls.some((c) => c.isError),
				).length;
				const summaryPreview =
					...
					totalPreview +
					" steps · " +
					(totalPreview - errorPreview) +
					"✓" +
					(errorPreview
						? ' <span style="color:var(--red)">' + errorPreview + "✗</span>"
						: "") +
					"</div>";
				const previewStepsHtml = renderStepListHtml(
					currentSteps,
					getActiveStepKey(),
					e.toolSources,
				);
				const previewMinimapHtml =
					typeof renderMinimapHtml === "function"
						? renderMinimapHtml(
								currentSteps,
								tok?.perMessage || null,
								-1,
			...
			currentSteps.forEach((s) => {
				if (s.type === "tool-group")
					s.calls.forEach((c) => {
						toolFreq[c.name] = (toolFreq[c.name] || 0) + 1;
					});
			});
			const totalSteps = currentSteps.filter(
				(s) => s.type === "tool-group",
			).length;
			const errorCount = currentSteps.filter(
				(s) => s.type === "tool-group" && s.calls.some((c) => c.isError),
			).length;
			const summaryHtml =
				'<div style="padding:4px 8px 6px;border-bottom:1px solid var(--border);font-size:11px;color:var(--dim)">' +
			...
			const stepsHtml = renderStepListHtml(
				currentSteps,
				activeKey,
				e.toolSources,
			);
			...
			commitDetailHtml(focusedHtml, function () {
				requestAnimationFrame(() => {
					...
					if (mm && sa) {
						layoutMinimapBlocks(mm);
						initMinimapInteractions(mm, sa);
					}
				});
		...
		case "mcp-tools": {
			...
			const filtered = req.tools
				? req.tools.filter((t) =>
						isMcp ? t.name.startsWith("mcp__") : !t.name.startsWith("mcp__"),
					)
				: null;
			if (filtered?.length) {
				const usageCount = allEntries[selectedTurnIdx]?.toolCalls || {};
				const sorted = [...filtered].sort(
					(a, b) => (usageCount[b.name] || 0) - (usageCount[a.name] || 0),
				);
				const tags = sorted
					.map((t) => {
						...
						const badge =
							cnt > 0
								? ' <span style="font-size:9px;background:var(--accent);color:#fff;border-radius:3px;padding:0 3px;margin-left:3px">' +
									cnt +
									"x</span>"
								: "";
						return (
							'<span class="tool-tag">' + escapeHtml(t.name) + badge + "</span>"
						);
					})
					.join("");
				inner =
					'<div class="detail-content"><div class="tool-grid">' +
					tags +
					"</div>" +
					'<details style="margin-top:8px"><summary style="color:var(--dim);cursor:pointer;font-size:11px">Full definitions</summary><pre>' +
					escapeHtml(JSON.stringify(filtered, null, 2)) +
					"</pre></details></div>";
			} else {
				inner = e.reqLoaded
					? '<div class="col-empty">No ' +
						(isMcp ? "MCP" : "core") +
						" tools</div>"
					: loading;
			}
		...
		case "skills": {
			...
			const sc = countSkillInvocations(
				req.messages,
				tok.contextBreakdown?.loadedSkills,
			);
			...
			if (detailLoadedSkills.length) {
				const loadedTags = detailLoadedSkills
					.map((name) => {
						const cnt = sc[name] || 0;
						return (
							'<span class="tool-tag" style="border-color:var(--purple);opacity:' +
							(cnt > 0 ? "1" : "0.45") +
							'">' +
							escapeHtml(name) +
							(cnt > 1
								? ' <span style="font-size:9px;background:var(--purple);color:#fff;border-radius:3px;padding:0 3px;margin-left:3px">' +
									cnt +
									"x</span>"
								: "") +
							"</span>"
				...
				const tags = sortedInvoked
					.map(
						([name, cnt]) =>
							'<span class="tool-tag" style="border-color:var(--purple)">' +
							escapeHtml(name) +
							(cnt > 1
								? ' <span style="font-size:9px;background:var(--purple);color:#fff;border-radius:3px;padding:0 3px;margin-left:3px">' +
									cnt +
		...
		case "raw-req":
			inner =
				req && Object.keys(req).length
					? '<div class="detail-content"><pre>' +
						escapeHtml(JSON.stringify(req, null, 2)) +
						"</pre></div>"
					: e.reqLoaded
						? '<div class="col-empty">No request data</div>'
						: loading;
			break;
		case "raw-res":
			inner = resEvents.length
				? '<div class="detail-content"><pre>' +
					escapeHtml(JSON.stringify(resEvents, null, 2)) +
					"</pre></div>"
				: e.reqLoaded
					? '<div class="col-empty">No response data</div>'
					: loading;
			break;
		default:
			inner = '<div class="col-empty">Unknown section</div>';
	}
...
function renderToolDetail(c) {
	...
	html +=
		...
		"</div>";
	html +=
		'<button class="detail-copy-btn" onclick="copyDetailContent(this)" title="Copy output">Copy</button>';
	html += "</div>";
	...
	switch (c.name) {
		case "Bash": {
			...
			const desc = inp.description
				...
				: "";
			return (
				'<div class="detail-meta"><code class="detail-cmd-block">$ ' +
				escapeHtml(cmd.split("\n")[0]) +
				(cmd.includes("\n") ? " ..." : "") +
				"</code>" +
				desc +
				"</div>"
		...
		case "Read":
			return (
				'<div class="detail-meta"><code>' +
				escapeHtml(inp.file_path || "") +
				"</code>" +
				(inp.offset
					? ' <span class="detail-tag">L' +
						inp.offset +
						(inp.limit ? "-" + (inp.offset + inp.limit) : "+") +
						"</span>"
					: "") +
				"</div>"
			);
		...
		case "Edit":
			return (
				...
				"</code>" +
				(inp.replace_all
					? ' <span class="detail-tag">replace_all</span>'
					: "") +
				"</div>"
			);
		case "Grep":
			return (
				...
				"/</code>" +
				(inp.glob
					? ' <span class="detail-tag">' + escapeHtml(inp.glob) + "</span>"
					: "") +
				(inp.path
					? " in <code>" +
						escapeHtml(inp.path.split("/").slice(-2).join("/")) +
						"</code>"
					: "") +
				"</div>"
			);
		...
		case "Agent":
			return (
				...
				escapeHtml(inp.description || (inp.prompt || "").slice(0, 80)) +
				(inp.subagent_type
					? ' <span class="detail-tag">' +
						escapeHtml(inp.subagent_type) +
						"</span>"
					: "") +
				"</div>"
			);
		case "WebSearch":
			return (
				'<div class="detail-meta"><code>' +
				escapeHtml(inp.query || "") +
				"</code></div>"
			);
		case "WebFetch":
			return (
				'<div class="detail-meta"><code>' +
				escapeHtml((inp.url || "").replace(/^https?:\/\//, "").slice(0, 60)) +
				"</code></div>"
			);
		case "TaskCreate":
			return (
				'<div class="detail-meta">' + escapeHtml(inp.subject || "") + "</div>"
			);
		case "TaskUpdate":
			return (
				'<div class="detail-meta">Task #' +
				escapeHtml(inp.taskId || "") +
				(inp.status
					? ' → <span class="detail-tag">' + escapeHtml(inp.status) + "</span>"
					: "") +
				"</div>"
			);
		default:
...
function renderToolInput(c) {
	...
	if (c.name === "Bash") {
		...
		if (inp.timeout && inp.timeout !== 120000)
			html +=
				'<div class="detail-meta-line">timeout: ' + inp.timeout + "ms</div>";
		html += "</div>";
		return html;
	}
	...
	if (metaTools.includes(c.name)) {
		return (
			'<details class="detail-input-details"><summary>INPUT JSON</summary><pre>' +
			escapeHtml(json) +
			"</pre></details>"
		);
	}
	...
	return (
		'<div class="content-block"><div class="type">INPUT</div><pre>' +
		escapeHtml(json) +
		"</pre></div>"
	);
}
...
function renderToolOutput(c) {
		...
		if (hasImage) {
			...
			for (const block of c.result) {
				if (block.type === "image" && block.source?.data) {
					...
					html +=
						'<div class="content-block"><div class="type">IMAGE</div>' +
						'<img src="data:' +
					...
					html +=
						'<div class="content-block"><div class="type">TEXT</div><pre>' +
						highlightCredentials(block.text) +
						"</pre></div>";
				}
			}
			return html;
	...
	let html =
		'<div class="content-block"><div class="type"' +
		errStyle +
		">" +
		label +
		' <span style="color:var(--dim);font-weight:normal">' +
		lines.length +
		" lines</span></div>";
...
function renderCollapsedContent(lines, textFn) {
	...
	return (
		...
		fn(head) +
		"</pre>" +
		'<div class="detail-collapse-bar" onclick="document.getElementById(\'' +
		id +
		"').style.display='block';this.style.display='none'\">⋯ " +
		hiddenCount +
		" lines hidden — click to expand</div>" +
		'<div id="' +
		id +
		'" style="display:none"><pre>' +
		fn(lines.slice(HEAD_LINES, -TAIL_LINES).join("\n")) +
		"</pre></div>" +
		"<pre>" +
		fn(tail) +
		"</pre>"
...
function renderErrorOutput(text) {
	...
	if (stackStart > 0) {
		...
		return (
			...
			(lines.length - stackStart) +
			" lines)</summary>" +
			'<pre style="color:var(--red);opacity:0.7">' +
			escapeHtml(stack) +
			"</pre></details>"
		);
	}
...
function renderThinkingDetail(thinking, durLabel) {
	...
	const formatted = paras
		.map((p) => '<p class="think-para">' + escapeHtml(p) + "</p>")
		.join("");
	return (
		'<div class="detail-tool-header">' +
		'<div class="detail-tool-title">🧠 Thinking' +
		durLabel +
		"</div>" +
...
function showImageOverlay(src) {
	...
	if (!overlay) {
		...
		overlay.style.cssText =
			"position:fixed;inset:0;background:rgba(0,0,0,0.85);z-index:300;display:flex;align-items:center;justify-content:center;cursor:zoom-out";
	...
	overlay.innerHTML =
		'<img src="' +
		src +
		'" style="max-width:90vw;max-height:90vh;border-radius:6px;box-shadow:0 4px 30px rgba(0,0,0,0.5)">';
