...
(function () {
	try {
		const el = document.getElementById("__proxy_config__");
		if (el && el.textContent)
			window.__PROXY_CONFIG__ = JSON.parse(el.textContent);
	} catch {}
})();
...
function switchTab(tab, forceDiff) {
	...
	document.getElementById("row2-dashboard").style.display =
		tab === "dashboard" ? "" : "none";
	document.getElementById("row2-usage").style.display =
		tab === "usage" ? "" : "none";
	document.getElementById("row2-sysprompt").style.display =
		tab === "sysprompt" ? "" : "none";
