...
- **Provider module: Grok (#313).** Third agent module beside `claude` / `codex` (`AGENT_PROVIDERS` + `OPENAI_WIRE_CLIENTS`). Shared OpenAI Responses parser; client-header routing to `UPSTREAMS.xai` (`cli-chat-proxy.grok.com`, override `XAI_BASE_URL` / `GROK_BASE_URL`); launcher `ccxray grok`. The Usage tab gains Grok proxy costs and a Weekly SuperGrok Limit account card. Module contract: `docs/provider-modules.md`. Multi-agent hub acceptance: `test/multi-agent-proxy.e2e.test.js`.
- **Session Weather (#320).** Five-level health indicator on session cards with an actionable overlay — context pressure, error/fault streaks, and other per-session signals folded into one glanceable verdict. Hardened across the release: clickable overlay links with swimlane zoom (#369), cold-session support (#367/#368), isomorphic weather module with injected context windows (#377), imported-session windows (#384), metas dedup (#385), context-numerator correctness (#387), enrichment propagation on every merge path (#388). Non-falsifiable advice removed from the overlay (#336, PR #455).
...
- **Per-turn tool calls (#427 Phases 1–2, ADR 0018).** Anthropic entries now carry `turnToolCalls` (this turn's calls, extracted from the response) alongside the cumulative conversation count, and UI consumers prefer the per-turn value. Known limit: OpenAI-wire entries (Codex/Grok) never set `turnToolCalls`, so their aggregation falls back to per-tool maxima and undercounts repeated same-tool calls — documented in ADR 0018, follow-up tracked. `ccxray rebuild-index --apply` backfills `turnToolFail` onto legacy lines (#443).
- **SECURITY.md (#414).** Documents the existing trust model — loopback trusted by default, auth token required elsewhere, `CCXRAY_LOOPBACK_REQUIRE_AUTH=1` to re-gate loopback behind a reverse proxy. Documentation only; no auth behavior changed.
...
- **Three parallel rate engines (#397 defect 2 Phase 0, PR #419).** cost-worker and importer now share `server/default-rates.js` `calculateCostSimple` instead of maintaining drifting copies — which also corrected stale offline rates the copies had drifted into (several Claude models, OpenAI/Grok cache-creation rates, `gpt-5.6` prefix matching, dated-model parity). Live-path unification is tracked in #459.
- **Turn failure rate inflated (#438, PRs #439/#441/#442).** The dashboard read the cumulative conversation-level `hasToolFail` as a per-turn signal (a Lie Factor measured at 44× in #441's audit). The metric now uses genuine per-turn data and hides itself when none exists.
...
- **cost-worker could never exit (#395/#396, #400/#402, #413, ADR 0015).** Orphaned usage-scanner workers accumulated on parent death (72 workers holding 4.5GB observed, #395); the first fix's disconnect listener then kept an IPC ref alive so the worker never exited (#400), fixed by unref (#402); importing the module for unit tests also hijacked the importer's lifecycle (#413). Now a pure batch process: drain-exit on success, side-effect-free import, enumerated env input surface.
- **CI failed without running tests (#297, PR #456).** `npm audit` sat before the test step in the same job, so one transitive advisory (`ip-address` ≤10.3.0, per the npm advisory) turned 8 consecutive runs red with the whole suite skipped — merges blocked and test regressions invisible. Audit is now a separate non-blocking job; the lockfile bumps `ip-address` to 10.4.0 (#457, functionally verified through a real SOCKS5 handshake).
...
- **Server weather fold uses the shared L1 subagent rule (#381, PR #394).** session-index now classifies with the shared `agent-classification.js` L1 agentKey rule; the client's L2–L4 layers still apply only client-side (documented divergence, ADR 0013).
- **Cross-session agent leakage (#361).** Independent `context_management` sessions no longer attach to unrelated sessions/projects — the session-inference heuristic fails closed.
- **False Fork lanes from same-conversation overlaps (#364/#365).** Jitter, re-sends, and rewinds within one conversation rendered as phantom parallel lanes; they now ride the main lane as event markers (amends ADR 0008).
- **Dashboard navigation dead-ends.** Cold sessions promote to hot when live data arrives (#330); collapsed sidebars recover instead of stranding you on a blank view (#358); the empty Turns column hides until a session is selected (#324); opening System Prompt before any turn arrived no longer throws and strands the loading skeleton.
...
- **Store encapsulation (#429).** All entry-push sites go through `store.registerEntry()` (push + index + aliases atomic); raw pushes elsewhere under `server/` are blocked by an audit test (ADR 0003 hardening).
