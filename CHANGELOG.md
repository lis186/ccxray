...
- **Session Weather (#320).** Five-level health indicator on session cards with an actionable overlay — context pressure, error streaks, cache health folded into one glanceable verdict. Hardened across the release: clickable overlay links with swimlane zoom (#369), cold-session support (#367/#368), isomorphic weather module with injected context windows (#377), imported-session windows (#384), metas dedup (#385), context-numerator correctness (#387), enrichment propagation on every merge path (#388). Non-falsifiable advice removed from the overlay (#336, PR #455).
