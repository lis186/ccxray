...
- **Session Weather (#320).** Five-level health indicator on session cards with an actionable overlay — context pressure, error streaks, cache health folded into one glanceable verdict. Hardened across the release: clickable turn navigation and lane focus from the overlay, cold-session support (#367–#369), isomorphic weather module with injected context windows (#377), imported-session windows (#384), metas dedup (#385), enrichment propagation on every merge path (#387/#388). Non-falsifiable advice removed from the overlay (#336, PR #455).
