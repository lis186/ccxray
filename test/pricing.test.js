...
describe('pricing', () => {
  describe('getModelPricing', () => {
    ...
    it('returns pricing for Grok CLI wire model ids via lag override', () => {
      // docs.x.ai/developers/models/grok-4.5 — Input $2 / Cached $0.50 / Output $6
      // Until LiteLLM lists xai/grok-4.5, buildPricingTable applies the override.
      const table = buildPricingTable({});
      assert.ok(table['grok-4.5'], 'grok-4.5 lag override must apply when LiteLLM lacks it');
    ...
    it('calculates cost for a typical grok-4.5 turn', () => {
      // Ensure table has the override (module default load may not have fetched)
      const table = buildPricingTable({});
  ...
  describe('LITELLM_LAG_OVERRIDES lifecycle', () => {
    ...
    it('stops applying override once LiteLLM lists a watched key (auto-return)', () => {
      // Simulate LiteLLM catching up with xai/grok-4.5
      const litellm = {
        'xai/grok-4.5': { input: 2.1, output: 6.1, cache_create: 0, cache_read: 0.51 },
      };
      const table = buildPricingTable(litellm);
      // bare mirror from xai/ + lag override sees litellmKeys present → does NOT overwrite
      assert.equal(table['grok-4.5'].input, 2.1, 'LiteLLM rate must win after catch-up');
      assert.equal(table['xai/grok-4.5'].input, 2.1);
      // grok-build still missing in this simulated table → override still active
      assert.equal(table['grok-build'].input, 1);
    });
    ...
    it('applyLagOverrides reports remove-override when LiteLLM has the key', () => {
      const { status } = (() => {
        ...
        const table = applyLagOverrides({
          ...
          'grok-4.5': { input: 2, output: 6, cache_create: 0, cache_read: 0.5 },
        });
        const pricing = require('../server/pricing');
        return { table, status: pricing.lastLagOverrideStatus };
      })();
      const grok45 = status.find(s => s.id === 'grok-4.5');
      assert.ok(grok45);
      assert.equal(grok45.active, false);
      assert.equal(grok45.action, 'remove-override');
    });
  });
