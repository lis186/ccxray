...
20. **Lane label gutter 升級為 3 行 agent 資訊塊**（使用者需求）：名字（server 偵測 agentLabel，無 system prompt 時 fallback `subagent-<model>`）／模型·ctx window／sysprompt 版本 chips（每個 coreHash 一顆、寬度制截斷、點 chip 鎖定該版本第一個 turn、↗ 跳 System Prompt 分頁）。collapsed 也全顯示——泳道圖的核心任務是跨 agent 掃視比較，資訊必須不展開可見。server 端把 KNOWN_AGENTS 偵測結果（`agentKey`/`agentLabel`）加進 entry summary + index.ndjson，舊資料由 restore 用 shared sys_ 檔反查 backfill
