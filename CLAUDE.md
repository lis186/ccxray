...
- **responseId merge: every store-push site dedups through the shared helper and keeps `responseIndex` + entryIndex aliases in sync** — cold-load/restore use `store.mergeByResponseId`, the live forward.js sites use `store.registerOrMerge`, and `trimEntries` must delete a canonical's `_mergedIds` aliases + its responseIndex slot. A push site that updates entryIndex but not responseIndex (or forgets alias handling) silently reintroduces duplicates or breaks a delta chain. OpenAI/WS entries carry no responseId (exempt) — @docs/decisions/0012-response-id-read-time-merge.md
