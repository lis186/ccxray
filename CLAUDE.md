...
- **agentKey main/subagent classification must use `isMainTurnByAgentKey` from `agent-classification.js`** in `entry-rendering.js`, `workflow-timeline.js`, and `server/session-index.js` `_upsert()` (window fold only — weather filters keep raw `isSubagent` because L1 alone over-includes without L2-L5). The Herdr plugin is a documented EXCEPTION — `mainDisplayTurns` must not use that predicate (it returns true for the `agentKey:'agent'` shape the badge exists to exclude) — but the plugin's two surfaces must then split main-agent from whole-session figures IDENTICALLY, or they report different numbers for one pane. ANCHORED on `mainDisplayTurns` in both `summarizeTurnGroup` and `missionControlRow`: model label, context window + ctx%, cache%, tool failures, prompt-change signals. Deliberately WHOLE-SESSION in both: cost, turn count, age/5m rate, and `latest`/`first` (freshness proves ccxray is still watching the pane, which a subagent turn does as well as a main one) — @docs/decisions/0005-agent-key-unreliable-shared-contract.md
