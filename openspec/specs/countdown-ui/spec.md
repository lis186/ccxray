# Countdown UI Specification

## Purpose

捨棄誤導性的 turns-left 預測，改以「事實 + 常數門檻」的事件驅動倒數呈現：cache TTL countdown（plan-aware）、auto-compact 視覺參考線（83.5%）、recent-gate 防止 historical session 染紅。涵蓋 plan config / auto-detection / quota panel / cache notification / cross-level visual harmonisation。
