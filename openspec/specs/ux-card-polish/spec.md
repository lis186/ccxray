# UX Card Polish Specification

## Purpose

Project / Session / Turn card 的 UX 修正集合，七個小型 gap bundled 一起 ship 因為都觸碰相同的 rendering surface（miller-columns.js / entry-rendering.js / style.css）。涵蓋 continue 圖示按鈕、cache TTL 文字格式、last-assistant preview、project dot tooltip、cost 兩位小數、turn time row 主從結構、`hit:` → `cache:` label rename。
