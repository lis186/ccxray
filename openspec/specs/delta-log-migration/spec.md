# Delta Log Migration Specification

## Purpose

`scripts/migrate-to-delta.js` 一次性轉換歷史 FULL `_req.json` 為 delta 格式。Dry-run by default、atomic temp+rename、subagent / inferred entry filtering、跨已轉換 delta 延續 chain、`--snapshot-n` cap、anchor breakdown counters、`safeParseFirst` 處理 legacy 雙 JSON。
